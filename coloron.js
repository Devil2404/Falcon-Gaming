class Game {
  ...
  intro() {
    TweenMax.killAll(); //TweenMax.to('.splash', 0.3, { opacity: 0, display: 'none', delay: 1 })
  ...
  * Display score
  ...
  start() {
    ...
    this.isRunning = 1; // Clean up the stick and ball holders
    // and generate new ones

    $('#sticks, .scene .ball-holder').html('');
    $('#score').text(this.score);
    this.generateSticks();
    this.generateBall(); // disables scene animations for Phones
    ...
    this.moveToStart();
    this.moveScene(); // reset timescale to normal as the game speeds up
...
class Animation {
  ...
  playTriangle(el) {
    ...
    triangle.staggerFromTo(el.find('.triangle'), 0.3, {
      ...
      scale: 1
    }, 0.03).staggerTo(el.find('.triangle'), 1.5, {
      cycle: {
        rotationY: [0, 360],
        rotationX: [360, 0]
      },
  ...
  playBlock(el) {
    ...
    block.staggerFromTo(el.find('.block'), 0.3, {
      ...
      scale: 1
    }, 0.03).staggerTo(el.find('.block .inner:not(.inner-2)'), 1, {
      cycle: {
        x: ["+200%", "-200%"]
      },
      repeat: -1,
